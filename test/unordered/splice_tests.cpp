...
bool node_handle_compare(
        NodeHandle const& nh,
        std::pair<
            BOOST_DEDUCED_TYPENAME NodeHandle::key_type const,
...
void insert_node_handle_unique(Container1& c1, Container2& c2) {
    ...
    BOOST_STATIC_ASSERT(boost::is_same<node_type, BOOST_DEDUCED_TYPENAME Container2::node_type>::value);

    typedef BOOST_DEDUCED_TYPENAME Container1::insert_return_type insert_return_type1;
    typedef BOOST_DEDUCED_TYPENAME Container2::insert_return_type insert_return_type2;
    ...
    while(!c1.empty()) {
        ...
        if (!count) {
            ...
            BOOST_TEST(r.position != c2.end());
            BOOST_TEST(test::get_key<Container2>(*r.position) == test::get_key<Container2>(v));
            BOOST_TEST(r.node);
...
struct hash_thing {
    std::size_t operator()(int x) const { return x * 13 + 5; }
};
