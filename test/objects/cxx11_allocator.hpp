...
#define BOOLEAN_CONSTANT(name, value_)                                         \
  ...
  {
    BOOLEAN_CONSTANT(propagate_on_container_swap, Flag::is_propagate_on_swap)
  };

  template <typename Flag> struct assign_allocator_base
  {
    BOOLEAN_CONSTANT(
      propagate_on_container_copy_assignment, Flag::is_propagate_on_assign)
  };

  template <typename Flag> struct move_allocator_base
  {
    BOOLEAN_CONSTANT(
      propagate_on_container_move_assignment, Flag::is_propagate_on_move)
  };
